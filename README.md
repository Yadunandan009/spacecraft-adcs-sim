...
is a software study built in [Basilisk](https://github.com/AVSLab/basilisk)
(AVS Lab, CU Boulder). No physical flywheel exists yet; its
...
- **New, simulation-only:**
  ...
  - Attitude estimators: TRIAD/QUEST (static) and optionally an MEKF
    (dynamic, gyro + vector measurements) — self-derived and
    self-implemented; `ahrs` is a reference/oracle to check results
    against, not the implementation source
...
See [docs/architecture.md](docs/architecture.md) for the Basilisk block
...
installing Basilisk and confirming its module/class API against the
installed version's docs before writing any block code.
